...
signed main(void){
    ...
    dp[2].assign(n+2,0);
    for(int i=1;i<=n;++i){
        dp[0][i] = max(dp[1][i-1] , dp[2][i-1]); // No purchase 
        dp[1][i] = max(dp[0][i-1] , dp[2][i-1]) + v[i].first; // purchase from first 
        dp[2][i] = max(dp[0][i-1] , dp[1][i-1]) + v[i].second; // purchase from second
    }
